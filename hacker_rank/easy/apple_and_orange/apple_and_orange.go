...
func countApplesAndOranges(s int32, t int32, a int32, b int32, apples []int32, oranges []int32) {
	...
	applesCount, orangesCount := getCountApplesAndOranges(s, t, a, b, apples, oranges)
	fmt.Println(applesCount)
	fmt.Println(orangesCount)
}

func getCountApplesAndOranges(s int32, t int32, a int32, b int32, apples []int32, oranges []int32) (int, int) {
	// s = start point of house
	// t = end point of house
	// a = apple tree
	// b = orange tree

	fallOnHouseApplesCount := 0
	...
	for _, distance := range apples {
		...
		appleFallsOnPoint := a + distance
		if appleFallsOnPoint >= s && appleFallsOnPoint <= t {
			// apple's falls point should be between s and t
			fallOnHouseApplesCount++
		}
	}

	fallOnHouseOrangesCount := 0
	for _, distance := range oranges {
		if distance > 0 {
			continue
		}

		orangeFallsOnPoint := b + distance
		if orangeFallsOnPoint >= s && orangeFallsOnPoint <= t {
			// orange's falls point should be between s and t
			fallOnHouseOrangesCount++
		}
	}

	return fallOnHouseApplesCount, fallOnHouseOrangesCount
}
